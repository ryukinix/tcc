...
\begin{displayquote}
  ...
  points. (Try to perform transduction, not induction.)~\cite{vapnik2006semi}

\end{displayquote}

Em tradução-livre: \blockquote{Não estime uma função se você precisa estimar
 valores em dados pontos. (Tente executar transdução, não
indução)}. Essa sugestão é uma evidência que reforça a motivação
deste trabalho. Ao desenvolver um algoritmo transdutivo para
...
diretamente baseada nas rotulações iniciais que o usuário forneceu. Na
Figura~\ref{fig:transductive-vs-inductive} contém uma ilustração comparativa entre aprendizado
indutivo e transdutivo.
...
como cor e textura. Na Figura~\ref{fig:slic}, uma execução do SLIC é
...
separados por determinadas distâncias e orientação na imagem $I$.
...
\begin{figure}[h!]
		...
		\centering
		\UFCfig{}{\fbox{\includegraphics[width=8cm]{figuras/lcu-iteration}}}{\Fonte{\citeonline{VerriNetworkUnfoldingMap2018}}}
\end{figure}
